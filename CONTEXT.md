...
**(superseded, kept for the record) A tool-to-Send signal is read from `$('tool')` inside a try/catch, never from staticData.** Send's menu condition learned this in production (exec 22878, 2 Sep): the expression sandbox does not share the Code sandbox's facilities, `$getWorkflowStaticData` evaluated to nothing there, and the graceful-degradation catch hid exactly the failure it was meant to survive. `$()` on a node that did not run throws, which the catch turns into false — that asymmetry is the whole mechanism.

**The options list is three rows, and that is a WhatsApp rendering fact, not a preference.** At most 3 reply buttons render inline; a 4th item collapses every menu into an English 'Choose an item' list button whose label is not ours. A balance row added 2 Sep did this to the owner's greeting and was reverted the same day. Balance is typed, never tapped. The lesson is pinned above ITEMS_3 in n8n_whatsapp_menu.py.

**A lost resident gets the options list, and the decision is layered: judgment first, net underneath.** Owner decision of 2 Sep ("idk" got a warm sign-off and a dead thread; his no-keyword-triggers rule given verbatim). `show_menu` is a Code tool the model calls on judgment; it flags `staticData.menu_exec` with the execution id and `Send` attaches the real rows under the model's own words — one message. When the model recites the flows in words instead (Gemini does, under vagueness), `Send` treats a reply naming 3+ of the four flows as the options message and attaches the rows anyway — the promise-guard doctrine, decide on the reply alone, reading only the bot's output. Not knowing what you need is not needing nothing: the goodbye is for someone actually done.

**Understanding is shown in what the bot does with what it was told, never announced — and the ban is on structures, not phrases.** Owner steer of 1 Sep night ("i was more focused in the robotic repetition like i hear you and i understand"). Three structures are named in the prompt without quoting any sentence: the content-free comprehension announcement, the playback of the resident's own words behind a comprehension verb, and the agree-to-soften opener on a repeated request (explain briefly why it is still missing, or ask differently, instead). The two-step lesson is load-bearing: banning only the content-free version made the model paraphrase the resident back — a rule against an empty phrase invites the padded phrase; target the act.

**A completed matter ends with an offer; a finished conversation ends with a parting; both are described in the prompt as ACTS — offer, thank, wish — never as sentences.** Owner decision of 1 Sep night, replacing the function of the canned follow-up menu two days after its form was removed. The closing paragraph's last clause is load-bearing: after the goodbye, no more questions, or the offer rule re-asks into a parted conversation. Measured 1 Sep late (two real threads + two probe arcs): three completion offers and four outros, no two alike; one outro answered the resident's ערב טוב in kind. That is what "not fixed" looks like when it works.
