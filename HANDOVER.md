...
- **BLOCKER FOUND AND FIXED IN THE REPO, NOT YET APPLIED (23 Sep): resolving a WhatsApp ticket in the dashboard was refused by the database and the page said nothing.** Migration 036's `queue_ticket_resolved_notice()` was an INVOKER function, so its insert into the RLS-protected `ticket_notices` ran as the signed-in staff member, was refused (`new row violates row-level security policy`), and rolled the whole UPDATE back — the ticket stayed open and no notice was ever queued. Only WhatsApp-opened tickets with a phone were affected (the trigger's own condition), so OXS tickets always resolved and it went unseen from 18 Sep; every non-OXS row in `requests` still has `updated_at == created_at`. Proof: `set local role authenticated; update requests set status='resolved' …; rollback`. Fix: `supabase/037_notice_security_definer.sql` (same body, `security definer`, `search_path` pinned; no insert policy — staff keep no direct write to the queue). **The owner must run `python scripts/supabase_migrate.py --apply`** (classifier refuses it here). The dashboard now surfaces a refused save instead of redrawing silently (`/tickets?saveFailed=<ref>`, `tickets.saveFailed`). Re-run the rolled-back simulation after applying, then set **255-1307-26** to resolved and watch `ticket_notices`.
