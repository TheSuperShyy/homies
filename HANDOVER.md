...
- **Epoch is 9.** 8 was the get_balance schema fix; 9 the closing-acts paragraph (offer more when a matter completes, part warmly when the resident is done, nothing after the goodbye). An outro must never contain the verbatim intro sentence — the echo-menu clause in `Send` would staple the menu to a goodbye. Checked in two live outros; neither did.
