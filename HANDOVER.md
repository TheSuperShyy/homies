...
- **WAITING ON TWO RUNS (23 Sep): the payment-link template `payment_link_he`, wording final (owner picked "a and b combined": the callback to the call plus the personal-link line).** Written in `docs/features/11-whatsapp-bot/templates.md` and wired in the Edge Function (`PAYMENT_TEMPLATE`, `syncedTemplates()`, `sendTemplate()`, the `outside_window` retry inside `send_payment_link`), but **not submitted and not deployed**: (1) `python scripts/wa_templates.py create payment_link_he --apply` submits it to Meta — a write to the client's WABA, owner only; (2) `python scripts/supabase_functions.py --apply` deploys the function — the classifier refuses it in-session. Until both are done the debt agent behaves exactly as before. **Risk to watch:** Meta may reject a template whose variable carries a URL; `wa_templates.py` prints `rejected_reason`. If it is rejected, the fallback is a dynamic-URL button, which needs the OXS link's host (one real minted link, never written down — `check_paylink.py --mint`) plus button support in both `wa_templates.py` and the Chatwoot send, so it is a piece of work, not a retry. **At cutover, recreate it on Homies' WABA** like every other template.
