...
- **OPEN, urgent: OpenRouter credits are at ~$0.20 of $115 (22 Sep 15:40 UTC).** Probes got HTTP 402; the WhatsApp bot (n8n model nodes), the dashboard typed chat and the payment-link line all run on `OPENROUTER_API_KEY`; the voice agents do not. Top-up is the owner's; check `GET https://openrouter.ai/api/v1/credits` and the last executions of `u2JjrbcNPYyyh3yl` before trusting any chat probe.
