...
- **OPEN, from the owner's 14:27 debt call (22 Sep): "the office will send it" has no work item.** When `send_payment_link` cannot deliver (Meta's 24-hour window closed -- the owner's last message was 48 h old; Meta's error is on Chatwoot message 1643), the agent gives the office number and nothing is filed: no ticket, no `notify_team`, only a `payment_links` row `requested / outside_window`. Two candidate fixes, owner to choose: a payment-link Meta template (same path as `ticket_resolved_he`, PENDING), and/or open a payment ticket + team note on `sent:false`. Same call: the model skipped `log_call_outcome` before the closing phrase. **Watch on the next call:** whether the first line says the name -- the transcript of that call has none (`צהריים טובים? מדבר מיכאל…`) where earlier calls had it; if Vapi renders the Liquid greeting before variableValues on `firstMessage`, move the debt line to a `greeting` variable from the dashboard.
