...
- **Every agreed payment link files a request (22 Sep, function v94, LIVE, probed).** `send_payment_link` (voice) inserts a `requests` row of type `payment`: **resolved** when the WhatsApp message went ("קישור לתשלום ועד הבית נשלח לדייר בוואטסאפ במהלך השיחה (…)"), **open** with the reason in Hebrew when it did not (Meta's 24-hour window, no contact, OXS gave no link…) so the office has the job; the agent gives the resident the request number with the office number only in that case. Proven with a probe: 255-1320-26 resolved, WhatsApp message 1647 delivered. Closes the 14:27 finding (Chatwoot message 1643 carries Meta's window error). Still worth doing later: a payment-link Meta template so the link itself goes outside the window (same road as `ticket_resolved_he`, PENDING). Same call: the model skipped `log_call_outcome` before the closing phrase. **Watch on the next call:** whether the first line says the name -- the transcript of that call has none (`צהריים טובים? מדבר מיכאל…`) where earlier calls had it; if Vapi renders the Liquid greeting before variableValues on `firstMessage`, move the debt line to a `greeting` variable from the dashboard.
