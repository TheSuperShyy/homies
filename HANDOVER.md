...
- **The "done" message exists (22 Sep): Meta template `ticket_resolved_he` on the test WABA (PENDING at Meta as of 22 Sep 01:00 UTC -- `python scripts/wa_templates.py` shows the verdict, `… chatwoot` shows when inbox 1 has synced it).** Path: `requests.status` → `resolved` on a WhatsApp ticket with a phone → trigger (036) → `ticket_notices` row → n8n `Homies — ticket notices` (`pvMuradcJOASapF0`, every 2 min) → `send_ticket_notices` (function v92) → Chatwoot template send as the bot. Source of truth for wording: `docs/features/11-whatsapp-bot/templates.md`; this is the second fixed message after the menu, by owner decision (Meta's 24-hour window). **First real send still owed:** once APPROVED and synced, set 255-1294-26 to resolved on the dashboard and watch `ticket_notices` (`sent`, `conversation_id 1`). Rows `failed` carry Meta's reason; five tries then left alone. **At cutover, recreate the template on Homies' WABA** (`wa_templates.py create ticket_resolved_he --apply` against the new ids).
