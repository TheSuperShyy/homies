...
- **Both voice agents greet by the hour (22 Sep; built, push pending the owner's go).** The first word of each opening line is a Liquid `{% %}` block Vapi renders at call start from Jerusalem time (before 05:00 שלום, 05–11 בוקר טוב, 12–16 צהריים טובים, 17:00+ ערב טוב); nothing is passed in, inbound phone calls get it too. Lines: `docs/assistant/demo-inbound.md` fence and `docs/features/10-debt-followup/prompt.md` `### הפתיחה`. `prompt_probe.py` and `voice-chat/route.ts` render the block themselves. **To push:** `N8N_BASE_URL= python scripts/vapi_sync.py inbound --apply`, then `python scripts/vapi_set_voice.py --apply` (the inbound sync resets the voice to Eyal), then `… debt --apply`; read back `firstMessage` on both. Listen on the first call: if the voice reads `assign h` aloud, Vapi did not render the line and the fallback is a `greeting` variable from the dashboard.
