...
- **Vercel tokens in `.env` (20 Sep): `VERCEL_TOKEN` is invalid; `VERCEL_API` is valid but belongs to `thesupershyy` (yulgatch123@gmail.com), not the team `team_s9vitdP1zEaGqJBiFe5M414L` that owns `homies-dashboard` (`prj_sUfK8Sf1aIEfhySAsvBJN5vSlLeG`).** Nothing here can set Vercel env; the owner does it in the Vercel UI. A token for the right scope would make `scripts/` able to.
